...
by `cargo-dist` and overwritten on every `dist init`.
...
`cargo-dist` (note the `dist init` comment in the file header). If you ever
run `dist init` again, it will **overwrite** the hand-written `test-unit` job
and the `needs: [test-unit]` dependency on `plan`. After a regeneration,
re-add both to keep the unit-test gate. The file carries a `HAND-WRITTEN
ADDITION` banner to flag this.
