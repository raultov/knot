...
impl SearchHybridContextTool {
    ...
    pub async fn handle(
        ...
        let enriched_context = enrich_with_relationships(
            &context,
            &entity_names,
            handler,
            repo_name,
...
fn format_entity(entity: &serde_json::Value) -> String {
    ...
    if let Some(count) = entity.get("type_usage_count").and_then(|v| v.as_i64()) {
        output.push_str(&format!("**Type Usage:** Referenced in {} location(s)\n", count));
