...
const KIND_LABELS: &[(EntityKind, &str)] = &[
...
impl std::fmt::Display for EntityKind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        // The label table covers every variant; a miss falls back to the
        // derived `Debug` name so a future variant can never render empty.
        match KIND_LABELS.iter().find(|(k, _)| k == self) {
...
mod tests {
    ...
    fn test_display_labels() {
        // KIND_LABELS pairs every variant with its snake-case label; these
        // spot-checks cover the families and the exception-prone cases.
        let cases = [
            (EntityKind::Class, "class"),
            (EntityKind::Interface, "interface"),
            (EntityKind::KotlinCompanionObject, "kotlin_companion_object"),
